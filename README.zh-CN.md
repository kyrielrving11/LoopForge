...
> **v1.12** — `npm install loopforge`。MCP 服务器（8 工具）+ Perception-Skill +
> 库 API + 验证门控 + 结构化评估 + 记忆系统集成（三阶段注入 + 回写）+
> 多 Agent 委托支持（AgentTool / Coordinator）。零运行时依赖。260 测试。Node.js ≥18。
...
第 2 轮及以后：
  LoopForge 编译新的 prompt — 跟第 1 轮不同，因为：
    ...
    - 进度被追踪到具体成功标准（P4）
    - 错误的假设被纠正了（P5）
    - 策略基于分层门控：Tier 1 始终可用，Tier 2 在检查点边界或连续失败后开放
  → Agent 在更丰富的上下文中执行
...
- **结构化事件日志** — 设 `LOOPFORGE_LOG=1` 后 stderr 输出 JSON 行事件。6 种事件：`round_complete`、`circuit_breaker`、`gate_contradicted`、`tier2_escalation`、`vault_write_error`、`session_start` / `session_end`。默认静默，零开销。
...
- **滚动摘要** — 近 5 轮跨轮知识蒸馏。
- **分层门控技术路由** — Tier 1 (zero-shot/few-shot/CoT) 始终可用。Tier 2 (step-back/least-to-most/ToT) 仅在检查点边界或连续失败后开放。无旋转——技术在轮次间保持稳定。
