...
- 716 → 731 → 750 → 783 tests. End-to-end suites now run against a
  machine-backed policy (a real passing verification command); the
  unverified-success suites explicitly exercise the `"warn"` path. The
  P0 additions added 19 tests; Round Contract V1 adds 33 more: parser
  leniency/caps, all four checks, R-C1/R-C2 enforcement ladders and rule
  order, scope-helper units, Current Task replacement + hash neutrality,
  eval-template restatement, and a full MCP-session flow.

## 3.2.1 (2026-08-29)

Bug-fix batch from the project audit — two high-severity defects plus the
medium-severity contract gaps they uncovered.

### Backtrack now actually exits

- **Redo submissions after a backtrack are evaluated, not replayed.** The
  transaction layer's idempotency replay (`readCommitted`) matched only the
  roundId; a committed backtrack decision was replayed for the redo
  submission (same roundId), so the agent's fix was silently discarded and
  the loop re-emitted the backtrack prompt forever. The advance path now
  skips backtrack entries (crash recovery still replays them via
  `recover()`).
- **`findSafeRestorePoint` no longer trusts compile-time lineage entries**
  (which hardcode `success: true` and carry no round_transaction) — restore
  points are chosen from committed feedback decisions, so dirty/rejected
  rounds are actually skipped and skipped-round discoveries are collected.
  `isCleanRound` now judges by the committed decision and error flags, not
  the agent's self-reported success (a progress-stall round is a valid
  restore target).
- **R4/R5 progress windows include the redo's own progress** after a
  backtrack (`progressWindow`) — the pre-rollback history otherwise keeps
  rejecting the redo forever and the loop dies in a spurious terminate.
- **`autoFeedback` no longer treats a committed backtrack as "already
  committed"** — the redo replaces the stale backtrack feedback entry, which
  was still poisoning the progress window.

### Contract fixes

- `SERVER_INSTRUCTIONS` no longer points at the nonexistent `loopforge_list`
  tool (use `loopforge_status view=all`).
- `loopforge_next` no longer requires the `evaluation` parameter at the
  schema level — the output-with-embedded-eval-block path is reachable again
  (the handler keeps the "either-or" check).
- `create()` pre-flights the cross-process lease BEFORE compiling (which
  writes round-1 lineage + state file), and rolls back its in-memory
  registration if the save-time lease race is lost — no vault pollution, no
  `loop_already_running` deadlock after a conflict.

### Compiler / prompt fixes

- L1 collapse's `(violated this round)` annotation works: current-round
  violations feed the constraint lifecycle, so `last_violated_at_round`
  reaches the current round. Decayed discovered constraints are
  **auto-reactivated** when violated again (previously a documented promise
  that was never wired).
- Emphasized hard constraints / success criteria are moved, not duplicated —
  they render in Critical Context and are removed from their source sections
  (the previous "zero token overhead" invariant was violated for these two
  sections).
- Sub-goal lifecycle fields (`completed_subtasks` / `blocked_subtasks` /
  `canceled_subtasks`) persist on committed feedback entries — the
  subgoal_drift check can finally see prior rounds' sub-goal state.
- The drift clarification streak is only touched when R7 actually
  participates in the decision — higher-priority rejections no longer
  advance it toward an early terminate.
