 ...
 */
import { makeLoopCompileRequest, makeLoopRoundResult, makeVaultConfig, } from "./protocol.js";
import { extractSelfEvaluation, heuristicSelfEvaluation, } from "./engine.js";
...
export function runOneRound(engine, config, currentRound, agentOutput) {
    ...
    if (agentOutput !== null && currentRound > 1) {
        ...
        if (selfEval !== null) {
            engine.autoFeedback(selfEval, config.loopId, currentRound - 1, // feedback is for the PREVIOUS round
            config.task);
        }
    ...
    if (context) {
        ...
        if (latest) {
            const lineage = (latest.loop_lineage ?? {});
    ...
    if (selfEval !== null) {
        ...
        if (fb.success && fb.constraint_violations.length === 0)
            lastQuality = 5;
        else if (fb.success && fb.constraint_violations.length === 0)
            lastQuality = 4;
        else if (fb.success)
            lastQuality = 3;
        else if (fb.constraint_violations.length > 0)
            lastQuality = 2;
        else
            lastQuality = 1;
    }
...
export async function runAutonomousLoop(engine, config, execute) {
    const maxRounds = config.maxRounds ?? 20;
    ...
    for (let round = 1; round <= maxRounds; round++) {
        ...
        // Execute the prompt
        try {
            agentOutput = await execute(roundResult.roundOutput.prompt, round);
        }
        catch {
            finalStopReason = "stalled";
            break;
    ...
    try {
        ...
        if (context) {
            ...
            if (results.length > 0) {
                ...
                healthAtStop = lineage.loop_health ?? null;
            }
        }
    }
    catch {
