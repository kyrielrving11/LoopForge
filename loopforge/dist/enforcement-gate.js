...
gitHead) {
    ...
    if (gitHead) {
        lines.push(`**Restore to commit \`${gitHead.slice(0, 12)}\`** — the verification ` +
            ...
            "accepting your next submission.", "", "```bash", "# Option C: If the failed rounds created commits, reset HEAD to the restore point", `git reset --hard ${gitHead.slice(0, 12)}`, "```", "", "> Option C discards the failed rounds' commits — use it only when those", "> commits belong to the failed work.", "");
    }
    lines.push(`**If git is unavailable**, manually revert the files listed above ` +
...
function enforcePrematureBoundary(flags, consecutiveRejections) {
    const flag = flags.find((f) => f.check === CHECK_PREMATURE_BOUNDARY && f.severity === "error");
    if (!flag)
        return null;
    if (consecutiveRejections >= 2) {
        return makeEnforcementResult({
            action: "terminate",
            reason: "Repeated premature Round Contract boundary claims — done_when items " +
    ...
    return makeEnforcementResult({
        action: "reject",
        reason: flag.detail,
        fix_instructions: "You claimed success under a Round Contract but done_when items are " +
...
function enforceScopeDrift(flags, selfEval, consecutiveRejections, vaultEntries = []) {
    ...
    if (consecutiveRejections >= 2) {
        return makeEnforcementResult({
            action: "terminate",
            reason: `Repeated scope drift without substantive drift_clarification — ` +
...
function enforceBacktrackNotRestored(flags, vaultEntries = [], currentRound = 0) {
    ...
    return makeEnforcementResult({
        ...
        fix_instructions: "Restore the working directory to the clean round's state " +
...
function extractAnchors(clarification) {
...
function clarificationIsSubstantive(clarification, selfEval, vaultEntries) {
    ...
    for (const anchor of anchors) {
        ...
        if ([...knownFiles].some((file) => file === anchor || file.endsWith(`/${anchor}`) || file.endsWith(`\\${anchor}`))) {
            return true;
        }
    }
    return false;
...
function buildClarificationRejectionNotice(streak) {
    const base = "Your declared next_action from the previous round does not match " +
...
function enforceIntentDrift(flags, selfEval, consecutiveRejections, driftClarificationStreak = 0, vaultEntries = []) {
    ...
    if (clarification && clarification.length >= 20 && !hasAnchor) {
        ...
        if (newStreak >= maxStreak) {
            return makeEnforcementResult({
                action: "terminate",
                reason: `Agent has submitted weak drift_clarification (no concrete IDs or ` +
    ...
    if (newStreak >= maxStreak) {
        return makeEnforcementResult({
            action: "terminate",
            reason: `Agent has drifted from its declared next_action for ${newStreak} ` +
    ...
    return makeEnforcementResult({
        action: "reject",
        reason: flag.detail,
        fix_instructions: "Your declared next_action from the previous round does not match " +
            "what you actually did this round. You must either: " +
