...
function edgeId(kind, from, to, version, round) {
...
function evidenceRelation(envelope, claim, reference) {
    if (reference.startsWith("file:")) {
        if (envelope.files.confidence === "verified" && envelope.files.value.includes(reference.slice(5)))
            return "supports";
        if (envelope.files.confidence === "contradicted")
            return "contradicts";
        return null;
    }
    if (reference.startsWith("check:")) {
        ...
        if (!check)
            return null;
        if (check.status === "failed" || envelope.checkProvenance?.[name]?.confidence === "contradicted")
            return "contradicts";
        if (check.status === "passed" && envelope.checkProvenance?.[name]?.confidence === "verified")
            return "supports";
        return null;
    }
...
function descendants(stepId, dependencies) {
    const found = new Set();
    const visit = (id) => {
        for (const [candidate, deps] of dependencies) {
            if (!deps.includes(id) || found.has(candidate))
                continue;
            found.add(candidate);
            visit(candidate);
        }
...
export function buildGovernanceGraph(loopId, entries, options = {}) {
    const nodes = new Map();
    const edges = new Map();
    const diagnostics = [];
    ...
    for (const entry of workflowEntries) {
        ...
        if (options.throughRound !== undefined && typeof storedRound === "number" && storedRound > options.throughRound)
            continue;
        ...
        if (options.throughRound !== undefined && event.round > options.throughRound)
            continue;
        events.push(event);
    }
    ...
    for (const version of versions) {
        ...
        if (version > versions[0]) {
            ...
            if (prior !== undefined)
                edges.set(edgeId("supersedes", versionId, `plan-v${prior}`, version), { id: edgeId("supersedes", versionId, `plan-v${prior}`, version), kind: "supersedes", from: versionId, to: `plan-v${prior}`, planVersion: version });
        }
        ...
        for (const step of plan.steps) {
            ...
            if (step.refinesStepId) {
                const sourceKnown = [...plans.entries()].some(([priorVersion, priorEvent]) => priorVersion < version && priorEvent.payload.plan.steps.some((candidate) => candidate.id === step.refinesStepId));
                if (!sourceKnown)
                    diagnostics.push({ code: "orphan_refinement", severity: "error", detail: `${step.id} refines missing ${step.refinesStepId}`, nodeId: step.id });
    ...
    for (const step of effectivePlan?.steps.filter((candidate) => candidate.status === "blocked") ?? []) {
        ...
        if (blocked.length)
            diagnostics.push({ code: "blocked_descendants", severity: "warn", detail: `${step.id} blocks ${blocked.length} descendant step(s)`, nodeId: step.id });
    }
    for (const event of events) {
        if (event.eventType === "plan_approval") {
            ...
            if (!nodes.has(planId))
                diagnostics.push({ code: "approval_version_mismatch", severity: "error", detail: `${approvalId} references missing plan v${event.payload.plan_version}`, nodeId: approvalId });
        ...
        if (event.eventType === "step_result") {
            ...
            for (const claim of envelope?.claims ?? []) {
                if (!nodes.has(claim.targetId))
                    diagnostics.push({ code: "orphan_claim", severity: "error", detail: `Unknown claim ${claim.targetId}`, nodeId: claim.targetId });
...
export function summarizeGovernanceGraph(graph) {
...
export function graphEntriesHash(entries) {
    return createHash("sha256").update(stableStringify(entries)).digest("hex");
}
export function buildGraphSlice(workflow, previous) {
    ...
    const blockedDescendantCount = active
        ? descendants(active.id, dependencies).filter((id) => !["done", "canceled"].includes(plan?.steps.find((step) => step.id === id)?.status ?? "pending")).length
        : 0;
    return {
...
//# sourceMappingURL=governance-graph.js.map