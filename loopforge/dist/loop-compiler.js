...
export function filterConstraintsForSubTask(allConstraints, subTask, threshold = 0.15) {
    if (!allConstraints.length || !subTask.trim())
        return [];
...
export function formatDelegationPrompt(subTask, subAgentType, relevantConstraints, options) {
    const lines = [];
    // Header
...
export function buildDelegationSummary(vaultContext) {
    if (!vaultContext)
        return "";
    const results = vaultContext.results;
    if (!results?.length)
        return "";
    const delegations = results.filter((r) => r?.task_type === "delegation_journal");
    ...
    for (const d of delegations) {
        const lineage = d.loop_lineage;
        const round = typeof lineage?.round === "number" ? lineage.round : 0;
        const dels = lineage?.delegations;
        if (!dels?.length)
            continue;
        for (const e of dels) {
            entries.push({
    ...
    for (const r of results) {
        ...
        if (lineage.loop_id === loopId && lineage.round === roundNum) {
            return {
                ...
                success: r.success ?? lineage.success ?? true,
...
function getPreviousRoundEvidence(loopId, roundNum, vaultContext) {
    if (vaultContext === null)
        return null;
    const results = vaultContext.results || [];
    for (const r of results) {
        ...
        if (lineage.round !== roundNum)
            continue;
        const evidence = (r.execution_evidence ?? lineage.execution_evidence);
        if (evidence) {
            return {
                progress_estimate: evidence.progress_estimate ?? 0,
                success_criteria_met: evidence.success_criteria_met ?? [],
            };
        }
        return null;
    }
...
function countProgressStallRounds(loopId, currentRound, vaultContext, threshold) {
    if (vaultContext === null || currentRound < 2)
        return 0;
    let count = 0;
...
function detectFailurePatterns(sampled) {
    if (sampled.length < 2)
        return [];
    const patterns = [];
    let runStart = -1;
...
function classifyRun(run) {
    ...
    for (let i = 0; i < tokensList.length - 1; i++) {
        for (let j = i + 1; j < tokensList.length; j++) {
            if (jaccard(tokensList[i], tokensList[j]) < 0.4)
                return null;
        }
    }
    ...
    return (`technique '${technique}' on task '${taskPreview}' ` +
...
function extractFailureRoundNums(patterns) {
    const nums = new Set();
    for (const p of patterns) {
        ...
        if (rangeMatch) {
            ...
            if (singleMatch)
                nums.add(parseInt(singleMatch[1], 10));
        }
    }
    return nums;
}
...
// ═══════════════════════════════════════════════════════════════════════════
export function buildRollingSummary(loopId, currentRound, vaultContext, sinceRound) {
    if (vaultContext === null)
        return null;
    const results = vaultContext.results || [];
    if (!results.length)
    ...
    for (const r of sampled) {
        if (r.output_summary) {
            ...
            keyOutcomes.push(`[R${r.round}] ${status} (${r.technique_used || "n/a"}): ` +
    ...
    if (failedRoundNums.size > 0 && recurringIssues.length > 0) {
        ...
        const failureViolations = new Set();
        const successViolations = new Set();
        ...
        recurringIssues.push(...[...violationCounts.entries()]
            .filter(([, count]) => count >= 2)
            .sort((a, b) => b[1] - a[1])
            .map(([v, count]) => {
...
export function formatRollingSummaryForPrompt(rs) {
    ...
    if (rs.failed_patterns && rs.failed_patterns.length) {
        ...
        lines.push("");
        lines.push("The following approaches failed repeatedly. The compiler has demoted " +
...
export function buildCheckpointSummary(selfEval, round, activeConstraints, retiredConstraints) {
    const policy = getPolicy();
    ...
    const label = selfEval.checkpoint_label?.trim() ||
...
export function formatCheckpointForPrompt(cs) {
    if (!cs)
        return "";
    // Must have at least a label and some content to be meaningful
    if (!cs.label && !cs.outcome && !cs.carried_constraints.length)
        return "";
    const lines = [
...
function loadLatestCheckpoint(loopId, currentRound, vaultContext) {
    if (!vaultContext)
        return null;
    const results = vaultContext.results || [];
    let latest = null;
    ...
    return makeCheckpointSummary({
        label: latest.label ?? "",
        declared_at_round: latest.declared_at_round ?? 0,
        outcome: latest.outcome ?? "",
        carried_constraints: latest.carried_constraints ?? [],
        resolved_constraints: latest.resolved_constraints ?? [],
    });
}
// ═══════════════════════════════════════════════════════════════════════════
...
export function compileL2(request, vaultContext) {
    ...
    if (lr?.compression_checkpoint === true) {
        ...
        checkpointSummary = loadLatestCheckpoint(request.loop_id, request.round, vaultContext);
        if (checkpointSummary) {
            checkpointRound = checkpointSummary.declared_at_round;
        }
    }
    // v3.5: Rolling summary — optionally scoped to post-checkpoint rounds
    const rollingSummary = buildRollingSummary(request.loop_id, request.round, vaultContext, checkpointRound > 0 ? checkpointRound : undefined);
    ...
    if (evidence && vaultContext) {
        ...
        if (prevEvidence) {
            ...
            if (delta < stallThreshold && evidence.progress_estimate < 0.95) {
                ...
                const stallCount = countProgressStallRounds(request.loop_id, request.round, vaultContext, stallThreshold);
                if (stallCount >= stallRounds) {
                    warnings.push(`P4: Progress has stalled — <${(stallThreshold * 100).toFixed(0)}% improvement for ${stallCount} consecutive rounds. Current: ${(evidence.progress_estimate * 100).toFixed(0)}%`);
                }
            }
        }
    }
