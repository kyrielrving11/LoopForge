...
function evolveConstraints(request, objective, previous) {
...
export function buildRollingSummary(loopId, currentRound, context, sinceRound = 0) {
    ...
    for (const entry of entries) {
        ...
        const summary = typeof entry.output_summary === "string"
            ...
            : typeof data.output_summary === "string" ? data.output_summary : "";
        if (summary)
            outcomes.push(`[R${round}] ${success === false ? "failed" : "accepted"}: ${summary}`);
...
export function alignTask(proposedTask, request, context) {
    ...
    const score = similarity(proposedTask, [objective.objective, ...objective.success_criteria, ...objective.hard_constraints].join(" "));
    return makeTaskAlignment({
...
function checkpoint(request, active, retired) {
    const last = request.last_round_result;
    if (!last?.compression_checkpoint)
        return null;
