...
import { activeContractOf, compileSessionContext, validateSubmission, } from "./submission-boundary.js";
// ── SessionManager ─────────────────────────────────────────────────────────
export class SessionManager {
    ...
    async advance(sessionId, output, submission, roundId) {
        ...
        const boundary = validateSubmission(this.loopStore, this.sessions.get(sessionId), submission);
        if (!boundary.ok) {
            return {
                sessionId,
        ...
        return this.withSessionQueue(sessionId, async () => {
            ...
            try {
                return await this.lifecycle.advance(sessionId, output, boundary.evaluation, roundId);
            }
            catch (error) {
