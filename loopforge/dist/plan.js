...
export function computeReadyStepIds(plan) {
    const done = new Set(plan.steps.filter((step) => step.status === "done" || step.status === "canceled").map((step) => step.id));
    return plan.steps
        .filter((step) => step.status !== "done" && step.status !== "canceled" && step.status !== "blocked" &&
...
export function validatePlan(plan, options) {
    ...
    if (!plan.objective.trim())
        errors.push("plan.objective must be non-empty");
    if (plan.steps.length === 0)
        errors.push("plan.steps must contain at least one step");
    if (plan.steps.length > options.maxSteps)
        errors.push(`plan.steps exceeds maximum ${options.maxSteps}`);
    ...
    if (missingRequiredConstraints.length) {
        ...
        for (const constraint of missingRequiredConstraints)
            addDiagnostic({
    ...
    for (const step of plan.steps) {
        ...
        ids.add(step.id);
        if (!step.title.trim())
            errors.push(`${step.id}: title must be non-empty`);
    ...
    for (const step of plan.steps) {
        for (const dependency of step.dependsOn) {
            ...
            if (dependency === step.id)
                errors.push(`${step.id}: step cannot depend on itself`);
        }
    }
    ...
    if (missingCriteria.length) {
        ...
        for (const criterion of missingCriteria)
            addDiagnostic({ code: "uncovered_success_criterion", severity: "error", path: "successCriteria", message: `Success criterion is not covered: ${criterion}`,
    ...
    if (missingConstraints.length) {
        ...
        for (const constraint of missingConstraints)
            addDiagnostic({ code: "uncovered_constraint", severity: "error", path: "constraints", message: `Plan constraint is not covered: ${constraint}`,
...
export function collectPlanRiskTags(plan) {
    ...
    if (matches(/\b(rm\s+-rf|reset\s+--hard|recursive delete|destructive)\b|删除工作区|清空工作区/))
        inferred.push("destructive_workspace");
    if (matches(/\b(database migration|schema migration|migrate database|migration)\b|数据迁移|[\\/._-]migrations?(?:[\\/._-]|$)/))
        inferred.push("data_migration");
    if (matches(/\b(production|prod deploy|deploy to prod|release to production)\b|(?:^|[\s\\/._-])prod(?:$|[\s\\/._-])|生产环境|上线部署/))
        inferred.push("production_change");
    if (matches(/\b(credentials?|permissions?|api keys?|secrets?)\b|凭据|密钥|权限/))
        inferred.push("credentials_or_permissions");
    if (matches(/\b(publish|send email|webhook|payment|external side effect)\b|对外发布|外部副作用/))
        inferred.push("external_side_effect");
    if (matches(/\b(public api|breaking change|api break)\b|公共接口|破坏性接口/))
        inferred.push("public_api_break");
    return [...new Set([...declared, ...inferred])]
        .filter((tag) => HIGH_RISK_TAGS.includes(tag));
}
export function criticalPlanChange(previous, next) {
    const stable = (value) => JSON.stringify(value);
    if (previous.objective !== next.objective)
        return true;
    if (stable(previous.successCriteria) !== stable(next.successCriteria))
        return true;
    if (stable(previous.constraints) !== stable(next.constraints))
        return true;
    ...
    return next.steps.some((step) => {
        ...
        const fields = outlineRefinement
...
function normalizedPlanForComparison(plan) {
    const sort = (values) => [...new Set(values.map((item) => item.trim()))].sort();
    return {
...
export function assessPlanChange(previous, next) {
    ...
    if (previous.objective !== next.objective) {
        changedPaths.push("objective");
        reasons.push("objective changed");
    }
    if (JSON.stringify(normalized(previous.successCriteria)) !== JSON.stringify(normalized(next.successCriteria))) {
        changedPaths.push("successCriteria");
        reasons.push("global success criteria changed");
    }
    if (JSON.stringify(normalized(previous.constraints)) !== JSON.stringify(normalized(next.constraints))) {
        changedPaths.push("constraints");
        reasons.push("hard constraints changed");
    }
    ...
    for (const id of new Set([...previousSteps.keys(), ...nextSteps.keys()])) {
        ...
        if (!old && current) {
            changedPaths.push(`steps.${id}`);
            reasons.push(`step ${id} added`);
            continue;
        }
        if (old && !current) {
            changedPaths.push(`steps.${id}`);
            reasons.push(`step ${id} removed`);
            continue;
        }
        if (!old || !current)
            continue;
        ...
        const fields = ["title", "kind", "dependsOn", "scope", "successCriteria", "constraints", "acceptanceCriteria", "evidenceRequirements", "refinement", "riskTags", "refinesStepId"];
        for (const field of fields) {
            if (JSON.stringify(old[field]) === JSON.stringify(current[field]))
                continue;
            changedPaths.push(`steps.${id}.${String(field)}`);
            if (field === "title" || field === "scope" || field === "refinesStepId")
                reasons.push(`tactical step detail changed: ${id}.${String(field)}`);
            else if (normalOutlineRefinement)
                reasons.push(`normal outline refinement: ${id}.${String(field)}`);
            else if (["acceptanceCriteria", "evidenceRequirements", "dependsOn", "successCriteria", "constraints"].includes(String(field)))
                reasons.push(`step contract changed: ${id}.${String(field)}`);
            else
                reasons.push(`step structure changed: ${id}.${String(field)}`);
        }
    }
    if (riskChanged)
        return { impact: "risk", changedPaths: [...new Set(changedPaths)].sort(), reasons: [...new Set([...reasons, "declared or inferred risk tags changed"])].sort() };
    ...
    const hasContractStepChange = [...previousSteps.keys()].some((id) => {
        ...
        const changed = (field) => JSON.stringify(old[field]) !== JSON.stringify(current[field]);
        if (normalOutlineRefinement) {
            return ["dependsOn", "successCriteria", "constraints", "riskTags"]
                .some(changed);
        }
        return ["kind", "dependsOn", "scope", "successCriteria", "constraints", "acceptanceCriteria",
            "evidenceRequirements", "refinement", "riskTags"].some(changed);
    });
    const normalRefinementOnly = !hasContractStepChange && reasons.every((reason) => /normal outline refinement|tactical step detail|step .* added/.test(reason));
...
export function validatePlanReplacement(previous, next) {
    ...
    for (const step of previous.steps.filter((candidate) => candidate.kind === "outline" && !["done", "canceled"].includes(candidate.status))) {
...
export function validatePlanReplacementDiagnostics(previous, next) {
    const diagnostics = [];
    const errors = validatePlanReplacement(previous, next);
    for (const error of errors) {
        const stepId = error.match(/^(ps-[^: ]+)/)?.[1];
        let code = "completed_step_mutation";
        let repairHint = "Restore the previous plan state and submit against the exact current baseVersion.";
        if (/unknown|does not exist/.test(error)) {
            code = "invalid_refinement";
            repairHint = "Reference an existing non-terminal outline from the exact baseVersion.";
        }
        else if (/inherit risk tags/.test(error)) {
            code = "risk_tag_not_inherited";
            repairHint = "Copy every inherited high-risk tag to the refinement child.";
        }
        else if (/outline cannot be removed/.test(error)) {
            code = "invalid_refinement";
            repairHint = "Keep the outline, cancel it with a reason, or add refinesStepId children.";
        }
...
export function deriveRefinementLinks(plan) {
...
export function createWorkflowState(planSource, baselineConstraints = [], approvalPolicy = "risk_only", planningProfile = "minimal") {
    return {
        phase: "planning",
        approvalPolicy,
        planningProfile,
...
export function buildPlanningPrompt(task, constraints, planningProfile = "minimal", diagnostics = []) {
    return [
        "# LoopForge Planning",
        "",
        "Mode: planning.",
...
export function buildRefinementPrompt(plan, step) {
    return [
...
export function buildPlanChangePrompt(plan, request, discoveries = []) {
    return [
...
export function buildAuditPrompt(plan) {
    return [
...
export function computeWorkflowProgress(workflow) {
    ...
    const covered = successCriteria.filter((criterion) => plan?.steps.some((step) => step.successCriteria.includes(criterion)));
    const provisionallyMet = successCriteria.filter((criterion) => {
        const owners = plan?.steps.filter((step) => step.kind !== "outline" && step.successCriteria.includes(criterion)) ?? [];
    ...
    if (workflow.phase === "planning")
        readiness = "planning";
    else if (workflow.phase === "awaiting_approval")
        readiness = "awaiting_approval";
    else if (workflow.phase === "auditing")
        readiness = "auditing";
    else if (workflow.phase === "terminal" && workflow.blockingGate)
        readiness = "awaiting_external_gate";
    else if (workflow.phase === "terminal" && successCriteria.every((criterion) => auditVerified.has(stableClaimId("cr", criterion))))
        readiness = "completed";
    else if (workflow.phase === "terminal")
        readiness = "blocked";
    else if (allImplementationDone && outlines.length === 0 && allGatesDone)
