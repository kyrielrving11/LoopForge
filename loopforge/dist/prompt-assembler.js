...
const VERIFICATION_ACTIONS = {
    ...
    intent_drift: "Explain the pivot in drift_clarification with concrete references, or redo the round as promised.",
    subgoal_drift: "Reconcile the sub-goal list with your reported work before resubmitting.",
...
function l1Sections(state, baseline, emphasized) {
    ...
    const activeSubs = state.subGoals.filter((sg) => sg.status === "in_progress" || sg.status === "pending");
...
function renderConfusionAlerts(points, state, expandAll) {
    if (points.length === 0)
        return "";
    const lines = [
        "## ⚠️ Confusion Alerts",
...
function renderExpandedSection(sectionName, state) {
    switch (sectionName) {
        case "milestones": {
            if (state.milestones.length === 0)
                return "";
            const text = state.milestones
                ...
                if (m.carried_constraints.length > 0)
                    lines.push(`  Carried: ${m.carried_constraints.slice(0, 3).join("; ")}`);
                if (m.resolved_constraints.length > 0)
                    lines.push(`  Resolved: ${m.resolved_constraints.slice(0, 3).join("; ")}`);
                return lines.join("\n");
            })
        ...
        case "constraint_lifecycle": {
            if (state.inactiveConstraints.length === 0 && state.retiredConstraints.length === 0)
                return "";
            const lines = [];
            if (state.inactiveConstraints.length > 0) {
            ...
            if (state.retiredConstraints.length > 0) {
                lines.push("**Retired:**");
                for (const c of state.retiredConstraints)
                    lines.push(`- ~${c}~`);
            }
        ...
        case "progress": {
            ...
            if (p.estimate === null && p.criteriaMet.length === 0 && p.criteriaRemaining.length === 0)
                return "";
            const lines = [];
            const total = p.criteriaMet.length + p.criteriaRemaining.length;
            if (total > 0)
                lines.push(`**Criteria**: ${p.criteriaMet.length}/${total} met`);
            if (p.estimate !== null)
                lines.push(`**Estimate**: ${(p.estimate * 100).toFixed(0)}%`);
            if (p.tests)
                lines.push(`**Tests**: ${p.tests.passed} passed, ${p.tests.failed} failed, ${p.tests.skipped} skipped`);
            if (p.filesChanged.length > 0) {
                lines.push("**Files Changed**:");
                for (const f of p.filesChanged.slice(0, 10))
                    lines.push(`- ${f}`);
            }
