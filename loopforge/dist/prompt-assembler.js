...
import { activeSubGoalView, buildPhaseLine, hashCanonicalState, milestoneHeading, } from "./canonical-state.js";
...
function activeWarnings(flags, currentRound) {
    const cutoff = currentRound - ACTIVE_WARNINGS_WINDOW;
    const shown = flags
        .filter((flag) => flag.rounds.some((round) => round >= cutoff))
        .sort((a, b) => Math.max(...b.rounds) - Math.max(...a.rounds) || b.count - a.count)
...
function truncateAtLine(text, room) {
...
function renderWithinBudget(sections, fixedText, budget) {
    ...
    const ranked = sections
        ...
        .sort((a, b) => (SECTION_PRIORITY[a.item.id] ?? DEFAULT_SECTION_PRIORITY) -
    ...
    while (prefix > 0) {
        ...
        if (rendered.length + total <= budget)
            break;
        prefix--;
    }
 ...
 *  guess where to look. */
function renderConfusionAlerts(points, fullDensity) {
    if (points.length === 0)
        return "";
    const lines = [
        "## ⚠️ Confusion Alerts",
