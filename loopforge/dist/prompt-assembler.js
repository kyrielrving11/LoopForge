...
function bulletsWithIds(values, prefix) {
...
function bulletsWithIdentities(values, metadata) {
    const idEnabled = getPolicy().evolution.constraint_id_enabled;
    if (!idEnabled)
        return bullets(values);
...
const VERIFICATION_ACTIONS = {
    ...
    success_without_verified_evidence: "Back the success claim with machine-verifiable evidence: run tests or a required verification command and report the actual output. If you genuinely performed no work, provide execution_report and declare no_change_reason.",
    ...
    duplicate_constraint_discovery: "Update the existing constraint entry instead of duplicating it.",
    recurring_violation: "Explain why this violation keeps recurring and try a different approach — do not repeat the failed strategy.",
    retract_fresh_constraint: "Justify the retraction explicitly, or restore the constraint.",
    evidence_integrity: "Correct the mismatch between your claims and the recorded evidence.",
    required_command_failed: "Re-run the required verification command and report its actual output before claiming success.",
    command_evidence_mismatch: "Report the true command output and reconcile the mismatch.",
    backtrack_workspace_not_restored: "Restore the skipped-round files and the expected git HEAD, then resubmit.",
    criteria_claims_unverified: "Provide evidence for each claimed success criterion.",
    ...
    round_scope_drift: "Revert the out-of-scope file changes, or close the active contract and declare the extended scope in a new proposal. Scope drift is a machine fact and is not waivable by explanation.",
    // v3.8 — item model: claimed-but-unverified debt
    contract_items_unverified: "Run the bound command(s) for each claimed item so they are observed passing, or report the item as remaining (or declare outcome=\"blocked\").",
    contract_premature: "The ACTIVE contract is still open — restate it unchanged to continue it; a different contract is ignored until every item is verified or the round reports blocked.",
    // v3.7.1 — opt-in gate layer: cited gates need an approved human decision
    user_gate_unresolved: "The round cites a high-risk action without an approved human decision. Run loopforge_gate_check, present the approval question, call loopforge_gate_resolve after the human decides, and resubmit with the approved gate id in evaluation.gate_ids.",
};
...
export function verificationActionFor(check) {
    return VERIFICATION_ACTIONS[check] ?? VERIFICATION_ACTION_FALLBACK;
}
function verificationText(state, level) {
    const lines = state.verificationFlags.map((flag) => {
        ...
        if (level === "l0" || flag.severity === "info")
            return base;
...
function constraintIdOf(text, metadata) {
    const meta = metadata.find((m) => m.text === text);
    return meta?.id ?? `c-${deriveItemId(text)}`;
}
export function diffConstraints(baseline, activeTexts, metadata, round, emphasized) {
    if (!baseline)
        return { changed: activeTexts, unchangedCount: 0, removedCount: 0 };
    ...
    for (const text of activeTexts) {
        ...
        if (!baselineIds.has(id) || violatedThisRound)
            changed.push(text);
        else
            unchangedCount++;
    }
    ...
    if (emphasized.size > 0) {
        ...
        for (const id of baselineIds) {
            if (!currentIds.has(id) && !emphasizedIds.has(id))
                removedCount++;
        }
    }
    else {
...
export function diffSubGoals(baseline, activeSubs) {
    if (!baseline)
        return { changed: activeSubs, unchangedCount: 0, removedCount: 0 };
    ...
    for (const sg of activeSubs) {
        ...
        if (prevStatus === undefined || prevStatus !== sg.status)
            changed.push(sg);
        else
            unchangedCount++;
    }
...
export function milestoneBoundaryChanged(baseline, milestones) {
    if (!baseline)
        return false;
    const baselineRanges = new Set(baseline.milestoneRanges.map(([s, e]) => `${s}:${e}`));
    return milestones.some((m) => !baselineRanges.has(`${m.round_range.start}:${m.round_range.end}`));
}
...
function l1Sections(state, baseline, emphasized) {
    ...
    if (active.length > 0) {
        const diff = diffConstraints(baseline, active, state.constraintMetadata, state.round, emphasized);
        const renderFull = !collapseEnabled || !baseline
            || diff.unchangedCount < COLLAPSE_MIN_UNCHANGED;
        if (renderFull) {
            sections.push({
                id: "active_constraints",
                text: section("Active Constraints / Success Criteria", bulletsWithIdentities(active, state.constraintMetadata)),
                mandatory: false,
            });
        }
        else {
            ...
            lines.push(`- … ${diff.unchangedCount} unchanged constraints, ${diff.removedCount} demoted since R${baseline.round} (see state file)`);
            sections.push({
                id: "active_constraints",
    ...
    if (activeSubs.length > 0) {
        const iconOf = (sg) => sg.status === "in_progress" ? "🔄" : sg.status === "blocked" ? "🚫" : "⏳";
        const diff = diffSubGoals(baseline, activeSubs);
        const renderFull = !collapseEnabled || !baseline
            || diff.unchangedCount < COLLAPSE_MIN_UNCHANGED;
        if (renderFull) {
            const maxShow = 5;
            const items = activeSubs.slice(0, maxShow).map((sg) => `${iconOf(sg)} [\`${sg.id}\`] ${sg.description}`);
            ...
            const items = diff.changed.slice(0, 5).map((sg) => `${iconOf(sg)} [\`${sg.id}\`] ${sg.description}`);
            items.push(`- … ${diff.unchangedCount} unchanged sub-goals, ${transitionCount + diff.removedCount} changed since R${baseline.round} (see state file)`);
            sections.push({
                id: "sub_goals",
                text: section("Active Sub-Goals", items.join("\n")),
                mandatory: false,
    ...
    if (state.recurringIssues.length > 0 || state.lessons.length > 0) {
        ...
        const body = violationLessons.length > 0
            ? violationLessons
                .map((lesson) => `- ${lesson.text} (violated ${lesson.count}×: R${lesson.rounds.join(", R")})`)
                .join("\n")
...
function l2Sections(state, fullStateMarkdown) {
    ...
    if (state.subGoals.length > 0) {
        ...
        lines.push(`─── ${view.total} total: ${view.activeTotal} active, ${view.done} done, ${view.canceled} canceled`);
        sections.push({
            id: "sub_goals_full",
    ...
    if (state.progress.estimate !== null ||
        ...
        if (state.criterionStatuses.length > 0) {
            ...
            lines.push(`**Machine (criteria)**: ${verifiedCount}/${state.criterionStatuses.length} verified` +
        ...
        if (state.progress.filesChanged.length > 0) {
            ...
            for (const f of state.progress.filesChanged.slice(0, 10))
                lines.push(`- ${f}`);
    ...
    if (state.agentTrustScore !== undefined) {
        ...
        if (state.agentTrustTrend.length > 0) {
            ...
            lines.push(`Trend (last ${state.agentTrustTrend.length}): ${state.agentTrustTrend.join(" → ")}`);
...
function matchEmphasize(targets, candidates, cap) {
...
function renderConfusionAlerts(points, state, fullDensity) {
    if (points.length === 0)
        return "";
    const lines = [
        "## ⚠️ Confusion Alerts",
...
export function assemblePromptArtifact(input) {
    ...
    let presentedState;
    if (levelIsL1) {
        const activeSubsForSnapshot = stateForSections.subGoals.filter((sg) => sg.status === "in_progress" || sg.status === "pending");
        presentedState = {
            ...
            subGoals: activeSubsForSnapshot.map((sg) => [sg.id, sg.status]),
            milestoneRanges: input.state.milestones.map((m) => [m.round_range.start, m.round_range.end]),
        };
    }
