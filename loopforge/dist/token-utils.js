...
export function tokenize(text) {
    ...
    let ascii = "";
    let cjk = [];
    const flushAscii = () => { if (ascii.length > 0) {
        tokens.add(ascii);
        ascii = "";
    } };
    const flushCjk = () => {
 ...
 *  here. */
export function machineGitMotionSeries(entries, currentRound, lookback) {
    ...
    for (const entry of entries) {
        ...
        if (!Array.isArray(roundEvidence))
            continue;
        const git = roundEvidence.find((value) => isRecord(value) && value.provider === "git");
        if (!git)
            continue;
