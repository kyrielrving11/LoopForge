...
function checkFilesIntegrity(selfEval, runtimeFilesChanged) {
    if (!runtimeFilesChanged)
        return null; // git unavailable, skip
    if (!selfEval.execution_evidence)
        return null; // heuristic fallback, skip
    ...
    if (reported.length === 0 && actual.length > 0) {
        return makeVerificationFlag({
            ...
            detail: `Agent reported no files changed but git shows: ${actual.join(", ")}`,
        });
    }
...
export function verifySelfEvaluation(selfEval, currentRound, vaultEntries, prevSelfEval = null, runtimeFilesChanged = null, evidenceSnapshots = []) {
    const flags = [];
