...
function canonical(path) {
    ...
    if (!existsSync(value))
        throw new Error(`workspace/store path does not exist: ${value}`);
    return realpathSync(value);
}
function normalized(path) {
    return canonical(path).replace(/[\\/]+$/, "").toLowerCase();
}
function isBroadRoot(path) {
...
export function resolveWorkspace(explicit) {
    ...
    if (isBroadRoot(candidate))
        throw new Error("workspaceRoot must not be a filesystem or user home root");
...
export function resolveStoreRoot(workspaceRoot, storeDir) {
    ...
    if (existsSync(chosen) && !statSync(chosen).isDirectory())
        throw new Error("storeDir must be a directory");
...
function configuredStoreDir(workspaceRoot) {
    try {
...
export function storeId(root) {
    return createHash("sha256").update(normalized(root)).digest("hex").slice(0, 32);
}
export class StoreLocator {
    path;
    constructor(path = resolve(homedir(), ".loopforge", "store-index.json")) {
        this.path = path;
    }
    entries() {
        try {
            const raw = JSON.parse(readFileSync(this.path, "utf8"));
            return Array.isArray(raw?.entries) ? raw.entries.filter((x) => Boolean(x && typeof x === "object")) : [];
        }
        catch {
            return [];
        }
    ...
    remember(binding) {
        ...
        try {
            ...
            return `Store locator unavailable: ${String(error)}`;
        }
        finally {
            try {
    ...
    candidates(workspaceId) { return this.entries().filter((entry) => entry.workspaceId === workspaceId); }
}
...
export class WorkspaceRuntime {
    ...
    locator;
    runtimeWarnings = [];
    constructor(prebind, locator = new StoreLocator()) {
        this.locator = locator;
        if (prebind?.workspaceRoot)
            this.bind(prebind.workspaceRoot, prebind.storeDir);
    }
    get isBound() { return this.binding !== null; }
    get warnings() { return [...this.runtimeWarnings]; }
    warn(message) {
        if (!this.runtimeWarnings.includes(message))
            this.runtimeWarnings.push(message);
    }
    get currentBinding() { return this.binding ? { ...this.binding, previousStoreRoots: [...this.binding.previousStoreRoots] } : null; }
    summary(status) {
    ...
    resolve(workspaceRoot, storeDir) {
    ...
    bind(workspaceRoot, storeDir) {
        return this.bindResolved(this.resolve(workspaceRoot, storeDir));
    }
    bindResolved(resolved) {
        const next = { schemaVersion: 1, workspaceId: resolved.workspace.id, workspaceRoot: resolved.workspace.root, workspaceFingerprint: resolved.workspace.fingerprint, storeId: storeId(resolved.storeRoot), storeRoot: resolved.storeRoot, previousStoreRoots: this.binding?.previousStoreRoots ?? [], boundAt: new Date().toISOString() };
        if (this.binding) {
            if (this.binding.workspaceId !== next.workspaceId)
                throw new Error("workspace_already_bound: workspace cannot be switched in a running MCP process");
            if (this.binding.storeId !== next.storeId)
                throw new Error("workspace_already_bound: store cannot be switched in a running MCP process; restart MCP with the requested storeDir");
        }
        this.binding = next;
    ...
    assertCompatible(binding) {
        if (!binding || typeof binding !== "object")
            return false;
        const value = binding;
        return value.workspaceId === this.binding?.workspaceId;
    }
    recordRelocation(previousStoreRoot) {
        if (!this.binding || resolve(previousStoreRoot).toLowerCase() === resolve(this.binding.storeRoot).toLowerCase())
            return;
        this.binding.previousStoreRoots = [resolve(previousStoreRoot), ...this.binding.previousStoreRoots]
            .filter((path, index, all) => all.indexOf(path) === index)
            .slice(0, 5);
    }
    diagnostic(code, warnings = [], matches = []) {
