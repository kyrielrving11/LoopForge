 ...
 * working-tree dist/ and loopforge-protocol.json. The optional --require-head
 * check additionally requires generated artifacts to already match HEAD; CI
 * can use that stricter gate after the clean-build comparison.
 *
...
try {
  ...
  // 4. Optionally require generated artifacts to match HEAD. This is kept
  // opt-in because local source changes legitimately make generated files
  // dirty until the change is staged/committed; the clean-build comparison
  // above is the useful default developer check.
  if (requireHead && existsSync(join(repoRoot, ".git"))) {
    try {
...
console.log(
  requireHead
    ? "verify:artifacts ✓ generated artifacts match a clean build and HEAD"
    : "verify:artifacts ✓ dist/ and loopforge-protocol.json match a clean build from src",
);
