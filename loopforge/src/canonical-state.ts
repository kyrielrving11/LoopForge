...
export const CANONICAL_STATE_SCHEMA_VERSION = 3 as const;
...
export function formatCompilationContext(
  context: PromptCompilationContext | null,
  fallbackTask: string,
): string {
  if (!context) return fallbackTask;
  const lines = [
    `Plan version: ${context.planVersion ?? "none"}`,
    `Prompt mode: ${context.promptMode}`,
  ];
  if (context.promptMode === "audit") lines.push("LoopForge Final Audit");
  if (context.activeStep) {
    lines.push(
      `Step: ${context.activeStep.id}: ${context.activeStep.title}`,
      `Kind: ${context.activeStep.kind}`,
    );
    if (context.activeStep.scope.length) lines.push(`Scope: ${context.activeStep.scope.join(", ")}`);
    if (context.activeStep.acceptanceCriteria.length) lines.push(
      "Acceptance criteria:",
      ...context.activeStep.acceptanceCriteria.map((item) => `- ${item}`),
    );
    if (context.activeStep.evidenceRequirements.length) lines.push(
      "Evidence requirements:",
      ...context.activeStep.evidenceRequirements.map((item) => `- ${item}`),
    );
  }
  if (context.evidenceGaps.length) lines.push(
    "Evidence gaps:",
    ...context.evidenceGaps.map((item) => `- ${item.id}: ${item.text}`),
  );
  if (context.failedChecks.length) lines.push(
    "Failed checks:",
    ...context.failedChecks.map((item) => `- ${item.name}: ${item.status}${item.summary ? ` - ${item.summary}` : ""}`),
  );
...
export function renderCanonicalStateMarkdown(state: CanonicalLoopState): string {
  ...
  if (state.evidence.checks.length) {
    addList(lines, "Checks", state.evidence.checks.map((check) =>
  ...
  if (state.verificationFlags.length) {
    addList(lines, "Verification", state.verificationFlags.map((flag) =>
  ...
  if (state.milestones.length) {
    ...
    for (const milestone of state.milestones) {
      lines.push(
        `### ${milestone.label}`,
        `Rounds ${milestone.round_range.start}-${milestone.round_range.end}`,
        "",
        milestone.outcome,
        "",
      );
  ...
  if (state.loopSynthesis) lines.push("## Loop Summary", "", state.loopSynthesis, "");
  if (state.externalContext) lines.push("## External Context", "", state.externalContext, "");
  if (state.graphSlice) {
    lines.push(
      "## Active Graph Slice",
      "",
      `Plan version: ${state.graphSlice.planVersion ?? "none"}`,
      `Active step: ${state.graphSlice.activeStepId ?? "audit"}`,
      `Parent outline: ${state.graphSlice.parentOutlineId ?? "none"}`,
      `Blocked descendants: ${state.graphSlice.blockedDescendantCount}`,
      "",
    );
  }
  return lines.join("\n").trimEnd() + "\n";
...
function planEvidenceGaps(request: LoopCompileRequest): string[] {
  if (!request.report_claim_targets?.length) return [];
