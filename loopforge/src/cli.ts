...
async function doctor(json: boolean, client?: InitClient, workflow = false, workspaceRoot?: string, storeDir?: string): Promise<number> {
...
function init(args: string[]): void {
  ...
  if (!result.registered) print(result.registration, typeof result.registration !== "string");
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  if (command === "doctor") {
    const client = option(args, "--client") as InitClient | undefined;
    if (client && !["claude", "codex"].includes(client)) throw new Error("doctor --client must be claude or codex");
