 ...
 *  approvals. */
export function preflightStructuredGate(action: GateActionDescriptor): StructuredGateVerdict {
  ...
  for (const effect of highRiskEffects) reasonCodes.push(`effects:${effect}`);
  if (action.reversibility === "irreversible") reasonCodes.push("reversibility:irreversible");
  if (action.reversibility === "unknown") reasonCodes.push("reversibility:unknown");
  if (action.authorization === "user_required") reasonCodes.push("authorization:user_required");
  if (action.authorization === "unknown") reasonCodes.push("authorization:unknown");
  if (action.effects.length === 0) reasonCodes.push("effects_empty");
  if (action.scope.length === 0) reasonCodes.push("scope_empty");
  ...
  const risk: StructuredGateVerdict["risk"] = userRequired
  ...
  return {
    ...
    ...(userRequired
      ...
      : {
          ...
          suggestedResolution:
            "Resolve the technical blocker and submit concrete evidence in the round evaluation.",
        }),
  };
}
