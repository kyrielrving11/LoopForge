...
function isCleanRound(round: CommittedRoundView): boolean {
...
export function buildBacktrackPrompt(
  ...
  recovery?: BacktrackRecoveryFacts,
): string {
  const lines: string[] = [
  ...
  if (triggerRule === "progress_stall") {
    lines.push(
      "### Why This Happened",
      "",
      `Progress stalled over rounds ${toRound + 1}–${fromRound}: the work ` +
      `produced no verifiable forward motion — no new files changed, no ` +
      `criteria met, no progress estimates moving.`,
      "",
      "The approach used in those rounds **did not work**. It should not be " +
  ...
  if (ownStreak >= 1) {
    ...
    if (getPolicy().engine.backtrack_enabled &&
      ...
      return makeEnforcementResult({
        action: "backtrack",
        reason:
          "The verification command's entrypoint is still not back to its " +
          "round-start state after being asked once, so this round cannot " +
          "produce a trustworthy observation.",
        fix_instructions:
          "After the rollback, restore the workspace as the Recovery Brief " +
    ...
    return makeEnforcementResult({
      action: "terminate",
      reason:
        "The verification command's entrypoint was changed in the round it " +
...
function enforceScopeDrift(
  ...
  if (scopeDriftWaiver) return null;
...
// v3.7: the flatline tier is an internal diagnostic of the single progress
// evaluator — both tiers emit the progress_stall enforcement check id.
type StallTier = "progress_stall" | "progress_flatline";

interface StallWindowVerdict {
  tier: StallTier;
  detail: string;
  window: number;
  threshold: number;
...
function evaluateStallWindow(
  ...
  if (machine !== null) {
    ...
    if (recent[0] < currentRound - window) return null;
    const values = recent.map((round) => progressByRound.get(round)!);
    if (values[values.length - 1]! >= 0.95) return null;
    if (tier === "progress_stall") {
      stalled = values.slice(1).every((value, index) =>
        value - values[index]! < threshold,
      );
...
function resolveStallDisposition(
  verdict: StallWindowVerdict,
  currentRound: number,
  vaultEntries: VaultEntry[],
  consecutiveRejections: number,
  lastRejectionCheck: string,
...
function enforceProgressStall(
  ...
  const verdict =
    evaluateStallWindow("progress_stall", selfEval, currentRound, vaultEntries, machineBackedSuccess)
...
interface RuleRow {
   ...
   *  rejection count (escalation notice on repeat, terminate at
   *  terminalAfter). Replaces the former UNLADDERED_REJECT_CHECKS name set.
   *  internal — the row's own enforcer implements its ladder.
   *  counter — fires only from the rejection counter (R6 catch-all). */
  ladder: "uniform" | "internal" | "counter";
  /** uniform rows only: prior same-check rejections after which the row
   *  terminates instead of rejecting again. */
...
for (const row of RULE_TABLE) {
  for (const id of row.checks) RULE_TABLE_BY_ID.set(id, row);
}
...
function enforceUserGateUnresolved(flags: VerificationFlag[]): EnforcementResult | null {
  const flag = flags.find(
    (f) => f.check === CHECK_USER_GATE_UNRESOLVED && f.severity === "error",
  );
  if (!flag) return null;
  return makeEnforcementResult({
    action: "reject",
    reason: flag.detail,
    fix_instructions:
      "The round depends on a high-risk action with no approved human " +
...
export function enforceRound(
  ...
  scopeDriftWaiver: boolean = false,
): EnforcementResult {
  ...
  const rules: Array<() => EnforcementResult | null> = [
    ...
    () => enforceVerificationEntrypointTampered(
      flags, vaultEntries, currentRound, consecutiveRejections, lastRejectionCheck),
    ...
    () => enforceSuccessWithoutVerifiedEvidence(
      flags, consecutiveRejections, lastRejectionCheck),
    // v3.3: R-C2 — files changed outside the contract's declared scope
    () => enforceScopeDrift(
      flags, consecutiveRejections, lastRejectionCheck, scopeDriftWaiver),
    // v3.8: verification debt — contract items claimed met without machine
    // verification (fires only after the configured streak)
    () => enforceContractItemsUnverified(
      flags, currentRound, vaultEntries, consecutiveRejections, lastRejectionCheck),
    // v3.7: single progress evaluator (former R4/R5 slots merged) —
    // reject → backtrack → terminate with the deadlock guard
    () => enforceProgressStall(
      selfEval, flags, currentRound, vaultEntries, consecutiveRejections,
      verifyResult.machineBackedSuccess, lastRejectionCheck),
    ...
    () => enforceMaxRejections(
      consecutiveRejections,
      scopeDriftWaiver && flags.some((f) => f.check === CHECK_ROUND_SCOPE_DRIFT)),
