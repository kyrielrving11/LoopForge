...
function getNodeDescription(node: ts.Node): string | undefined {
  ...
  if (ts.isJSDoc(first)) {
    if (typeof first.comment === "string") return first.comment.trim();
...
function convertEnum(node: ts.EnumDeclaration): SchemaNode {
  const values: string[] = [];
...
function collectRefNames(node: SchemaNode | undefined, out: Set<string>): void {
  ...
  if (node.properties) {
    for (const child of Object.values(node.properties)) collectRefNames(child, out);
  }
  if (node.items) collectRefNames(node.items, out);
  if (Array.isArray(node.anyOf)) {
    for (const child of node.anyOf) collectRefNames(child, out);
  }
}
...
function typeIncludesNull(typeNode: ts.TypeNode | undefined): boolean {
...
function convertInterface(
  ...
  for (const member of node.members) {
    ...
    const isOptional =
      !!member.questionToken || typeIncludesNull(member.type);
    if (!isOptional) {
...
function generateSchema(): void {
    ...
    for (const sf of program.getSourceFiles()) {
      ...
      for (const stmt of sf.statements) {
        ...
        if (name && missing.includes(name)) {
          ...
          addStatementDef(stmt, checker, defs);
          if (Object.keys(defs).length > before) added = true;
        }
      }
    }
    if (!added) break; // nothing more resolvable — stop
  }
  ...
  const schema: SchemaRoot = {
    ...
    description:
      "JSON Schema for the LoopForge loop-compile protocol. " +
