...
function destination(options: InitOptions): string {
  if (options.target) return resolve(options.target, "loopforge", "SKILL.md");
...
function registration(client: InitClient, options: Partial<InitOptions> = {}): string | Record<string, unknown> {
  const command = process.execPath;
  const args = [cliPath, "mcp"];
  if (options.workspaceRoot) args.push("--workspace", resolve(options.workspaceRoot));
  if (options.storeDir) args.push("--store-dir", options.storeDir);
...
function legacySkillPath(options: InitOptions): string {
  return join(dirname(destination(options)), "..", "perception", "SKILL.md");
}

export function removeManagedLegacySkillFile(
  pathInput: string,
  warnings: string[],
  managedDigests: ReadonlySet<string> = MANAGED_PERCEPTION_DIGESTS,
): void {
...
function removeManagedLegacySkill(options: InitOptions, warnings: string[]): void {
  removeManagedLegacySkillFile(legacySkillPath(options), warnings);
}

function registerClient(options: InitOptions, warnings: string[]): { registered: boolean; verified: boolean } {
  if (!options.register || options.client === "generic") return { registered: false, verified: false };
  ...
  if (options.workspaceRoot) serverArgs.push("--workspace", resolve(options.workspaceRoot));
  if (options.storeDir) serverArgs.push("--store-dir", options.storeDir);
...
export function initializeClient(options: InitOptions): InitResult {
