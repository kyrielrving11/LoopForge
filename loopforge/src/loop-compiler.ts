...
import { decidePromptLevel, type PromptLevelDecision } from "./prompt-policy.js";
...
function contextEntries(context: Record<string, unknown> | null): Entry[] {
  ...
  return context.results.filter((item): item is Entry =>
...
function roundReport(entry: Entry): Entry | null {
  const value = entry.round_report;
  return value && typeof value === "object" && !Array.isArray(value) ? value as Entry : null;
}
...
export function getPreviousRound(
  ...
  const compileLineage = [...entries].reverse()
    ...
    .find(({ candidate, data: candidateLineage }) =>
      entryRound(candidate) === round &&
  ...
  const status = ["completed", "blocked", "in_progress"].includes(String(report.status))
    ? report.status as PreviousRound["status"]
    : "in_progress";
  return {
    round,
    ...
    constraints_active: Array.isArray(data.constraints_active)
      ? data.constraints_active.filter((item): item is string => typeof item === "string") : [],
    summary: typeof report.summary === "string" ? report.summary : "",
  };
}

function latestObjective(loopId: string, context: Record<string, unknown> | null): LoopObjective | null {
...
function objectiveFor(request: LoopCompileRequest, context: Record<string, unknown> | null): LoopObjective {
...
function constraintProjection(objective: LoopObjective, planConstraints: string[]): {
  active: string[];
...
function workflowMilestones(entries: Entry[]): MilestoneSummary[] {
  const milestones: MilestoneSummary[] = [];
  for (const entry of entries) {
    const data = lineage(entry);
    const event = typeof data.event_type === "string" ? data.event_type : "";
    if (!["step_result", "plan_updated", "plan_submitted", "backtrack", "audit_completed"].includes(event)) continue;
...
function buildExecutionHistory(loopId: string, context: Record<string, unknown> | null): ExecutionHistorySummary {
  const entries = loopEntries(loopId, context);
  const reports = entries.map(roundReport).filter((item): item is Entry => item !== null);
...
function levelDecision(request: LoopCompileRequest, context: Record<string, unknown> | null): PromptLevelDecision {
...
export function decideLevel(request: LoopCompileRequest, context: Record<string, unknown> | null): "l0" | "l1" | "l2" {
  return levelDecision(request, context).level;
}

export function buildRoundReportBlock(request: Pick<LoopCompileRequest, "report_mode" | "report_claim_targets" | "attempt">): string {
...
export function compileLoop(request: LoopCompileRequest, context: Record<string, unknown> | null): LoopCompileResponse {
  ...
  const adaptiveL2 = policy.prompt.l2_adaptive_enabled
    ? Math.min(policy.prompt.l2_max_chars + request.round * policy.prompt.l2_adaptive_round_factor +
        history.milestones.length * policy.prompt.l2_adaptive_milestone_factor,
