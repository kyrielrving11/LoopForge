...
import type { MachineStatus, PresentedStateSnapshot, RoundStat } from "./canonical-state.js";
import {
...
import { deriveItemId, STABLE_ID_RE, jaccardSimilarity, unique, entryRound, isRecord, machineGitMotionSeries } from "./token-utils.js";
...
export function readPresentedBaseline(
  loopId: string,
  round: number,
  context: Record<string, unknown> | null,
): PresentedStateSnapshot | null {
  ...
  if (
    !Array.isArray(constraintIds) || !constraintIds.every((v) => typeof v === "string") ||
    !Array.isArray(subGoals) || !subGoals.every(
      (v) => Array.isArray(v) && v.length === 2 &&
        typeof v[0] === "string" && typeof v[1] === "string",
    ) ||
    !Array.isArray(milestoneRanges) || !milestoneRanges.every(
      (v) => Array.isArray(v) && v.length === 2 &&
  ...
  return {
    ...
    subGoals: subGoals as Array<[string, string]>,
    milestoneRanges: milestoneRanges as Array<[number, number]>,
  };
}
...
function entryExecutionEvidence(entry: Entry): Record<string, unknown> | null {
...
function entryCriteriaMet(entry: Entry): string[] {
...
function entryCriteriaRemaining(entry: Entry): string[] {
  const ev = entryExecutionEvidence(entry);
  if (!ev) return [];
...
export function deriveLessons(
  ...
  for (const entry of entries) {
    ...
    const viols = Array.isArray(entry.constraint_violations)
      ? entry.constraint_violations.filter((v): v is string => typeof v === "string")
      : [];
...
function deriveRoundStats(
  ...
  for (const entry of entries) {
    ...
    if (!existing || tid.endsWith(":feedback")) byRound.set(rnd, entry);
  }
  ...
  for (const rnd of rounds) {
    ...
    if (progress !== null) prevProgress = progress;
  }
  return stats;
}
...
function deriveActiveContract(
  ...
  const evaluations: CommittedRoundEvaluation[] = [];
  for (let rnd = 1; rnd < currentRound; rnd++) {
    ...
    evaluations.push({
      round: rnd,
      proposal: isRecord(contract) ? contract as unknown as RoundContract : null,
...
function entryActiveConstraints(entry: Entry): string[] {
  const lin = lineage(entry);
...
function entryRetractedConstraints(entry: Entry): string[] {
...
function entryProgressEstimate(entry: Entry): number {
  const ev = entryExecutionEvidence(entry);
  if (!ev) return 0;
 ...
 *  `mid.length > budget`. */
function sampleMiddleMilestones(
  mid: MilestoneSummary[],
  budget: number,
): MilestoneSummary[] {
  if (budget <= 0) return [];
  if (mid.length <= budget) return mid;
  const chosen: MilestoneSummary[] = [];
  const used = new Set<MilestoneSummary>();
  for (const m of mid) {
...
function sampleMilestones(
  ...
  if (level === "l2") return milestones;
  if (milestones.length <= policy.max_milestones) return milestones;
  ...
  if (midBudget === 0) return milestones.slice(0, head).concat(milestones.slice(milestones.length - tail));
...
export function buildRollingSummary(
  ...
  if (policy.enable_loop_synthesis && currentRound > 1) {
    ...
    const activeCount = completedEntries.length > 0
      ...
      : 0;
    const totalResolved = cappedMilestones.reduce(
      (sum, m) => sum + m.resolved_constraints.length,
      0,
    );
    loopSynthesis =
...
export function isConstraintId(ref: string): boolean {
...
function entryCompletedSubtasks(entry: Entry): string[] {
...
function entryBlockedSubtasks(entry: Entry): string[] {
...
function entryCanceledSubtasks(entry: Entry): string[] {
...
function findDiscoveredRound(
  ...
  for (const entry of vaultEntries) {
    ...
    const discovered = Array.isArray(entry.discovered_constraints)
      ? entry.discovered_constraints.filter((v: unknown) => typeof v === "string")
      : [];
    for (const d of discovered) {
      // Phase 1: exact ID match (v2.11)
      if (d === targetId || deriveConstraintId(d) === targetId) {
        if (earliest === 0 || rnd < earliest) earliest = rnd;
        break;
      }
...
function manageConstraintLifecycle(
...
): { active: string[]; inactive: string[]; metadata: ConstraintMeta[] } {
  const policy = getPolicy().evolution;
  ...
  for (const text of active) {
    ...
    const status: "active" | "inactive" =
      roundsSinceRelevant > policy.constraint_inactive_rounds
        ? "inactive"
        : "active";
...
export function checkLoopHealth(
  ...
  const drift =
    alignment.alignment_score < getPolicy().evolution.progress_mismatch_threshold ||
...
function levelDecision(
  ...
  const recoveryBoundary = loopEntries(request.loop_id, context)
    .some((entry) =>
      lineage(entry).committed_action === "backtrack" &&
  ...
  const adaptiveL2 = policy.prompt.l2_adaptive_enabled
    ? Math.min(
        policy.prompt.l2_max_chars
          ...
          + (subGoals.length * policy.prompt.l2_adaptive_subgoal_factor),
        policy.prompt.l2_adaptive_max_chars,
      )
    : policy.prompt.l2_max_chars;
