...
export function computeProjectHash(gitRoot: string): string {
...
export function detectClaudeMem(): { memoryDir: string; indexPath: string } | null {
  const gitRoot = findGitRoot();
  if (!gitRoot) return null;
  const hash = computeProjectHash(gitRoot);
  const memoryDir = join(homedir(), ".claude", "projects", hash, "memory");
  const indexPath = join(memoryDir, "MEMORY.md");
  if (!existsSync(memoryDir) || !existsSync(indexPath)) return null;
...
function buildQueryTerms(ctx: MemoryProviderContext): string[] {
  const terms = new Set<string>();
  for (const w of keywords(ctx.task)) terms.add(w);
  if (ctx.domain) for (const w of keywords(ctx.domain)) terms.add(w);

  if (ctx.phase === 2) {
    for (const issue of ctx.accumulatedContext.recurringIssues) {
      for (const w of keywords(issue)) terms.add(w);
    }
    for (const pat of ctx.accumulatedContext.failedPatterns) {
      for (const w of keywords(pat)) terms.add(w);
    }
  } else if (ctx.phase === 3) {
    for (const crit of ctx.accumulatedContext.remainingCriteria) {
      for (const w of keywords(crit)) terms.add(w);
    }
    for (const lesson of ctx.accumulatedContext.keyLessons) {
      for (const w of keywords(lesson)) terms.add(w);
    }
  }
...
function retrieveFromFiles(
  ...
  if (files.length === 0 || queryTerms.length === 0) return "";
...
export function createMemoryProvider(
  memoryDir: string,
): (ctx: MemoryProviderContext) => Promise<string> {
  return async (ctx: MemoryProviderContext): Promise<string> => {
...
function writeMemoryFile(
  ...
  name: string,
  description: string,
  memType: "project" | "feedback" | "reference",
  body: string,
): string {
...
export function createMemoryWriter(
  memoryDir: string,
  indexPath: string,
): (payload: LoopMemoryWriteback) => Promise<void> {
  return async (payload: LoopMemoryWriteback): Promise<void> => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      if (acquireLock(lockDir)) break;
      if (attempt === 2) return; // could not acquire after 3 attempts — skip
    ...
    try {
      ...
      const projectFile = writeMemoryFile(
        memoryDir,
        projectName,
        payload.projectEntry.title.slice(0, 100),
        "project",
        projectBody,
      );
      indexLines.push(
        `- [LoopForge: ${payload.task.slice(0, 60)}](${projectFile}) — ${payload.outcome} (${date})`,
      );
      ...
      for (let i = 0; i < payload.feedbackEntries.length; i++) {
        ...
        indexLines.push(
          `- [LoopForge Feedback](${fbFile}) — ${payload.outcome} (${date})`,
        );
      }
      ...
      if (payload.referenceEntry) {
        ...
        const refFile = writeMemoryFile(
          memoryDir,
          refName,
          payload.referenceEntry.description.slice(0, 100),
          "reference",
          refBody,
        );
        indexLines.push(
          `- [LoopForge Reference](${refFile}) — ${payload.outcome} (${date})`,
        );
      }
...
export function autoConfigureMemory(mgr: SessionManager): void {
