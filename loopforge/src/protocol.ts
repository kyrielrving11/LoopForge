...
export type PlanDiagnosticCode =
  ...
  | "invalid_refinement"
  | "stale_plan_version"
  | "completed_step_mutation"
  | "risk_tag_not_inherited"
  | "no_ready_step";
...
export interface NormalizedRoundEvaluation {
  ...
  materialAdvancement?: { material: boolean; signals: string[]; stepId?: string };
...
export interface StoreResolutionDiagnostic {
  ...
  searchedStores: Array<{ storeRoot: string; storeId: string; exists: boolean; stale?: boolean }>;
  matches: Array<{ storeRoot: string; loopId: string; workspaceId?: string | null }>;
  orphanMarkdownPaths: string[];
  recommendation: {
    action: "retry_resume" | "restart_mcp" | "inspect_store" | "restore_json" | "start_new_loop";
    toolCall?: { name: "loopforge_resume"; arguments: Record<string, unknown> };
    command?: { executable: string; args: string[]; display: string };
  } | null;
  warnings: string[];
}

// Internal compiler request types
...
export function makeLoopObjective(overrides: Partial<LoopObjective> = {}): LoopObjective {
  return {
    objective: "",
    success_criteria: [],
    hard_constraints: [],
...
// Round reporting and derived compiler state
...
export function makeExecutionHistorySummary(
  overrides: Partial<ExecutionHistorySummary> = {},
...
export function makeRoundHistoryEntry(
...
// Internal compiler response types
...
// Runtime result and enforcement types
