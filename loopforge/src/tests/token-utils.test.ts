...
describe("tokenize", () => {
  ...
  it("handles Chinese characters", () => {
    // Consecutive CJK characters form a single token
    const result = tokenize("修复重入漏洞");
    assert.ok(result.has("修复重入漏洞"));
  });

  it("handles CJK Extension A", () => {
    // Extension A chars are within 㐀-鿿 range
    const result = tokenize("㐀㐁㐂");
    assert.ok(result.has("㐀㐁㐂"));
  });
...
describe("jaccardSimilarity", () => {
  ...
  it("returns 1 when either set is empty (prevents false-positive dedup)", () => {
    assert.equal(jaccardSimilarity("", ""), 1);
    assert.equal(jaccardSimilarity("hello", ""), 1);
    assert.equal(jaccardSimilarity("", "world"), 1);
  });
  ...
  it("handles Chinese text similarity", () => {
    // Consecutive CJK chars form one token, so these are entirely different
    const score = jaccardSimilarity("修复重入漏洞", "修复重入漏洞");
    assert.equal(score, 1);
    // Different CJK strings with no shared tokens → 0
