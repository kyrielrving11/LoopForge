...
import { PromptCraftEngine, createEngine, } from "../engine.js";
import { AgentStatus, Mode, makeExecutionFeedback, } from "../protocol.js";
...
describe("Engine — Initialisation", () => {
    ...
    it("createEngine returns PromptCraftEngine instance", () => {
        const engine = createEngine("skills");
        assert.ok(engine instanceof PromptCraftEngine);
...
describe("Engine — Build mode (internal)", () => {
    ...
    it("returns OK with generated prompt", () => {
        ...
        assert.ok(result.response.prompt.includes("PromptCraft Build"));
...
describe("Engine — Review mode", () => {
    ...
    it("returns report when hydrate results provided", () => {
        ...
        const result = engine.handleReview(makeRequest({ mode: Mode.REVIEW }), hydrateResults);
        assert.equal(result.status, AgentStatus.OK);
