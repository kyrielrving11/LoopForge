 ...
 *
 * Python reference: backends/fs.py (~380 lines)
 */
...
function buildYamlFrontmatter(metadata: Record<string, unknown>): string {
  ...
  for (const line of fmBlock.split("\n")) {
    ...
    if (line.startsWith("  ") && !line.startsWith("    ")) {
      ...
      if (nestedMatch && currentKey) {
        ...
        (metadata[currentKey] as Record<string, unknown>)[nKey] =
...
function lineageDirName(loopId: string): string {
...
// ═══════════════════════════════════════════════════════════════════════════
// Markdown lineage I/O (module-level functions — match Python fs.py helpers)
// ═══════════════════════════════════════════════════════════════════════════

export function readLineageMd(
  loopId: string,
  roundNum: number,
  vaultPath?: string,
): { full_prompt: string; metadata: Record<string, unknown> } | null {
...
export function writeLineageMd(
  loopId: string,
  roundNum: number,
  content: string,
  metadata: Record<string, unknown>,
...
export function scanLineageMd(
  ...
  try {
    ...
    const files = readdirSync(dir).filter(
      (f) => f.startsWith("r") && f.endsWith(".md"),
    );
...
export class FSBackend implements VaultBackend {
  ...
  constructor(
    ...
    globalVaultPath = "~/.promptcraft/global_vault.json",
  ) {
